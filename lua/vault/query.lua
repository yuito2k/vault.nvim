...
function M.execute_pg_query()
  ...
  -- Helper: convert pgmoon values to display strings
  local function pg_val_to_str(val)
  ...
  -- Helper: fetch ordered column names from information_schema
  local function get_columns(table_name)
    local col_result, _ = pg:query(string.format([[
      SELECT column_name
    ...
    if col_result and col_result[1] then
      local headers = {}
      for _, col in ipairs(col_result) do
  ...
  for _, sql in ipairs(statements) do
    ...
    if not result then
      -- pgmoon returns nil + err string on failure
      local clean_err = tostring(err):match('[Ee]rr[or]*:%s*(.+)$') or tostring(err)
      table.insert(state.result_sets, { headers = { 'Error' }, rows = { { clean_err } } })
      had_error = true

    elseif type(result) == 'table' and result[1] then
      -- SELECT with rows returned
      local table_name = sql:match('[Ff][Rr][Oo][Mm]%s+"?(%w+)"?')
      local headers = table_name and get_columns(table_name) or nil

      -- Fallback to row keys if information_schema failed
      if not headers then
        headers = {}
        for k in pairs(result[1]) do table.insert(headers, k) end
        table.sort(headers)
      ...
      for _, row_data in ipairs(result) do
        local row = {}
        for _, h in ipairs(headers) do
          table.insert(row, pg_val_to_str(row_data[h]))
        end
        table.insert(rows, row)
      end
      total_rows = total_rows + #rows
  ...
  local stmt_word = #statements == 1 and '1 statement' or (#statements .. ' statements')
  state.last_query_status = string.format('Executed %s in %.2fms', stmt_word, total_ms)

  -- Auto-refresh explorer on DDL
  local function is_ddl(s)
    local up = s:match('^%s*(.-)%s*$'):upper()
    return up:match('^CREATE%s+TABLE')
        or up:match('^DROP%s+TABLE')
        or up:match('^ALTER%s+TABLE')
  end
  ...
  -- Save to history (system SQLite db, unchanged)
  if state.is_connected and state.db_id then
    local hist_db = require('sqlite.db'):open(state.sys_db)
    for _, sql in ipairs(statements) do
      local trimmed = sql:match('^%s*(.-)%s*$')
...
function M.old_execute_pg_query()
  local q_ovr_win = nil
  local r_ovr_buf = nil
  for id, name in pairs(state.wins) do
    if name == 'q_overlay' then q_ovr_win = id end
  ...
  for _, sql in ipairs(statements) do
    ...
    elseif type(result) == 'table' and result[1] then
      -- SELECT: result is a list of row maps
      local headers = {}
      for k in pairs(result[1]) do table.insert(headers, k) end

      -- PostgreSQL column order via information_schema
      ...
      if table_name then
        ...
        if col_result and type(col_result) == 'table' then
          for _, col in ipairs(col_result) do
            col_order[col.column_name] = tonumber(col.ordinal_position)
          end
        end
      end
    ...
    elseif type(result) == 'table' and not result[1] then
      ...
      if #headers == 0 then headers = { 'Result' } end
      table.insert(state.result_sets, { headers = headers, rows = { { 'Empty' } } })  -- TODO: prints empty on update, delete, insert cmds also
