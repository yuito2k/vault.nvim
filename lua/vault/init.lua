...
local function update_ui_state()
	...
	for win_id, name in pairs(state.wins) do
		if api.nvim_win_is_valid(win_id) and not name:find("scroll") and name ~= "overlay" and name ~= "bottom_bar" then
			...
			api.nvim_win_set_option(
				win_id,
				"winhl",
				"Normal:Normal,FloatBorder:"
					.. (active and state.hl_active_border or state.hl_inactive_border)
					.. ",FloatTitle:"
					.. title_hl
			)

			local title_text = ({ explorer = " [e] Explorer ", query = " [q] Query ", results = " [r] Results " })[name]
				or ""
	...
	if b_buf then
		...
		api.nvim_buf_set_lines(
			...
			1,
			false,
			{ mode_text .. " | e: Explorer | q: Query | r: Results | Esc: Close" }
		)
		api.nvim_buf_set_lines(b_buf, 1, 2, false, { "n: Normal | i: Insert" })
		api.nvim_buf_add_highlight(
			b_buf,
			-1,
			(is_insert and state.hl_mode_insert or state.hl_mode_normal),
			0,
			0,
			#mode_text
		)
...
local connect_db = function(ovr_buf)
	...
	state.open_nodes =
		{ ["MyLocalDB"] = true, ["Tables"] = false, ["Views"] = false, ["Indexes"] = false, ["Triggers"] = false }
...
M.open_db_float = function()
	...
	-- Explorer
	local exp_win = api.nvim_open_win(api.nvim_create_buf(false, true), true, {
		relative = "win",
	...
	local ovr_win = api.nvim_open_win(ovr_buf, false, {
		relative = "win",
		win = exp_win,
		width = exp_w - 4,
		height = main_h - 2,
	...
	local q_win = api.nvim_open_win(q_buf, false, {
		relative = "win",
		win = state.parent_win_id,
		width = q_w,
		height = math.floor(main_h * 0.6) - 3,
		col = exp_w + 2,
		row = 0,
		border = "rounded",
	})
	...
	api.nvim_create_autocmd(
		{ "WinEnter", "CursorMoved", "ModeChanged", "InsertEnter", "WinScrolled" },
		{ group = "DbViewEvents", callback = update_ui_state }
	)
