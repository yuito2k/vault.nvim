  ...
  for i, field in ipairs(conn_state.pg_fields) do
    ...
    vim.keymap.set('n', '<CR>', function()
      if field.type == 'dropdown' then
        ...
        M.show_dropdown_picker(field, conn_state.pg_wins[i])
      else
        vim.cmd 'startinsert!'
      end
    end, bopts)
...
function M.render_edit_connection_ui(db_id, record)
  ...
  local edit_state = {
    ...
    pg_fields = {
      ...
      { name = ' Port ',     value = tostring(record.port),      type = 'input', row = 12, col = 59, width = 22 },
  ...
  local function trigger_update_connection()
    ...
    if record.type == 'SQLite' then
      ...
      local update_query = string.format(
        [[UPDATE database SET name='%s', path='%s' WHERE id='%s';]],
        typed_name:gsub("'", "''"),
        typed_path:gsub("'", "''"),
        db_id:gsub("'", "''")
      )
    ...
    elseif record.type == 'PostgreSQL' then
      ...
      local db = require('sqlite.db'):open(path)
      local update_query = string.format(
        [[UPDATE database SET name='%s', path='%s', host='%s', port='%s', database='%s', username='%s', password='%s' WHERE id='%s';]],
        db_name:gsub("'","''"),
        conn_str:gsub("'","''"),
        server:gsub("'","''"),
        port:gsub("'","''"),
